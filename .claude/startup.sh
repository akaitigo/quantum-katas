#!/usr/bin/env bash
# =============================================================================
# セッション起動ルーチン（原則6: セッション間状態管理）
#
# セッション開始時に実行し、ツールのインストールとヘルスチェックを行う。
# 状態管理は git log + GitHub Issues で行う（CONTEXT.json/progress.json はオプション）。
#
# オプション:
...
# 3. CONTEXT.json — 現在の目標と次のアクション（オプション: 存在する場合のみ読み込み）
...
if [ -f "CONTEXT.json" ]; then
  ...
  echo "Last checks: $(jq -r '.last_checks // "no data"' CONTEXT.json)"
else
  echo "INFO: CONTEXT.json not found. Using git log + GitHub Issues for state management."
fi

# 4. progress.json — セッション履歴と直近の状態（オプション: 存在する場合のみ読み込み）
...
if [ -f "progress.json" ]; then
  ...
  echo "INFO: progress.json not found. Using git log for session history."
fi
...
# 7. セッション開始時刻を環境変数として記録（session-end.sh で使用）
export SESSION_START_TIME=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
echo ""
echo "=== Session started at $SESSION_START_TIME ==="
echo "Run 'bash session-end.sh \"summary\" \"next action\"' when done."
